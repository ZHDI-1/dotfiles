...
```sh
"$(chezmoi source-path)/scripts/bootstrap.sh" --mode core
"$(chezmoi source-path)/scripts/bootstrap.sh" --mode dev
"$(chezmoi source-path)/scripts/bootstrap.sh" --mode full --without-gui
```
